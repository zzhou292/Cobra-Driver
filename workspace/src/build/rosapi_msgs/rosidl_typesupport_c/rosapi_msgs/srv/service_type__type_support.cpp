// generated from rosidl_typesupport_c/resource/idl__type_support.cpp.em
// with input from rosapi_msgs:srv/ServiceType.idl
// generated code does not contain a copyright notice

#include "cstddef"
#include "rosidl_runtime_c/message_type_support_struct.h"
#include "rosapi_msgs/srv/detail/service_type__struct.h"
#include "rosapi_msgs/srv/detail/service_type__type_support.h"
#include "rosapi_msgs/srv/detail/service_type__functions.h"
#include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/message_type_support_dispatch.h"
#include "rosidl_typesupport_c/type_support_map.h"
#include "rosidl_typesupport_c/visibility_control.h"
#include "rosidl_typesupport_interface/macros.h"

namespace rosapi_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _ServiceType_Request_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _ServiceType_Request_type_support_ids_t;

static const _ServiceType_Request_type_support_ids_t _ServiceType_Request_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _ServiceType_Request_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _ServiceType_Request_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _ServiceType_Request_type_support_symbol_names_t _ServiceType_Request_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, rosapi_msgs, srv, ServiceType_Request)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, rosapi_msgs, srv, ServiceType_Request)),
  }
};

typedef struct _ServiceType_Request_type_support_data_t
{
  void * data[2];
} _ServiceType_Request_type_support_data_t;

static _ServiceType_Request_type_support_data_t _ServiceType_Request_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _ServiceType_Request_message_typesupport_map = {
  2,
  "rosapi_msgs",
  &_ServiceType_Request_message_typesupport_ids.typesupport_identifier[0],
  &_ServiceType_Request_message_typesupport_symbol_names.symbol_name[0],
  &_ServiceType_Request_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t ServiceType_Request_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_ServiceType_Request_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &rosapi_msgs__srv__ServiceType_Request__get_type_hash,
  &rosapi_msgs__srv__ServiceType_Request__get_type_description,
  &rosapi_msgs__srv__ServiceType_Request__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace rosapi_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, rosapi_msgs, srv, ServiceType_Request)() {
  return &::rosapi_msgs::srv::rosidl_typesupport_c::ServiceType_Request_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "rosapi_msgs/srv/detail/service_type__struct.h"
// already included above
// #include "rosapi_msgs/srv/detail/service_type__type_support.h"
// already included above
// #include "rosapi_msgs/srv/detail/service_type__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace rosapi_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _ServiceType_Response_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _ServiceType_Response_type_support_ids_t;

static const _ServiceType_Response_type_support_ids_t _ServiceType_Response_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _ServiceType_Response_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _ServiceType_Response_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _ServiceType_Response_type_support_symbol_names_t _ServiceType_Response_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, rosapi_msgs, srv, ServiceType_Response)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, rosapi_msgs, srv, ServiceType_Response)),
  }
};

typedef struct _ServiceType_Response_type_support_data_t
{
  void * data[2];
} _ServiceType_Response_type_support_data_t;

static _ServiceType_Response_type_support_data_t _ServiceType_Response_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _ServiceType_Response_message_typesupport_map = {
  2,
  "rosapi_msgs",
  &_ServiceType_Response_message_typesupport_ids.typesupport_identifier[0],
  &_ServiceType_Response_message_typesupport_symbol_names.symbol_name[0],
  &_ServiceType_Response_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t ServiceType_Response_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_ServiceType_Response_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &rosapi_msgs__srv__ServiceType_Response__get_type_hash,
  &rosapi_msgs__srv__ServiceType_Response__get_type_description,
  &rosapi_msgs__srv__ServiceType_Response__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace rosapi_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, rosapi_msgs, srv, ServiceType_Response)() {
  return &::rosapi_msgs::srv::rosidl_typesupport_c::ServiceType_Response_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
// already included above
// #include "rosidl_runtime_c/message_type_support_struct.h"
// already included above
// #include "rosapi_msgs/srv/detail/service_type__struct.h"
// already included above
// #include "rosapi_msgs/srv/detail/service_type__type_support.h"
// already included above
// #include "rosapi_msgs/srv/detail/service_type__functions.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
// already included above
// #include "rosidl_typesupport_c/message_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_c/visibility_control.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"

namespace rosapi_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{

typedef struct _ServiceType_Event_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _ServiceType_Event_type_support_ids_t;

static const _ServiceType_Event_type_support_ids_t _ServiceType_Event_message_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _ServiceType_Event_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _ServiceType_Event_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _ServiceType_Event_type_support_symbol_names_t _ServiceType_Event_message_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, rosapi_msgs, srv, ServiceType_Event)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_introspection_c, rosapi_msgs, srv, ServiceType_Event)),
  }
};

typedef struct _ServiceType_Event_type_support_data_t
{
  void * data[2];
} _ServiceType_Event_type_support_data_t;

static _ServiceType_Event_type_support_data_t _ServiceType_Event_message_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _ServiceType_Event_message_typesupport_map = {
  2,
  "rosapi_msgs",
  &_ServiceType_Event_message_typesupport_ids.typesupport_identifier[0],
  &_ServiceType_Event_message_typesupport_symbol_names.symbol_name[0],
  &_ServiceType_Event_message_typesupport_data.data[0],
};

static const rosidl_message_type_support_t ServiceType_Event_message_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_ServiceType_Event_message_typesupport_map),
  rosidl_typesupport_c__get_message_typesupport_handle_function,
  &rosapi_msgs__srv__ServiceType_Event__get_type_hash,
  &rosapi_msgs__srv__ServiceType_Event__get_type_description,
  &rosapi_msgs__srv__ServiceType_Event__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace rosapi_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(rosidl_typesupport_c, rosapi_msgs, srv, ServiceType_Event)() {
  return &::rosapi_msgs::srv::rosidl_typesupport_c::ServiceType_Event_message_type_support_handle;
}

#ifdef __cplusplus
}
#endif

// already included above
// #include "cstddef"
#include "rosidl_runtime_c/service_type_support_struct.h"
// already included above
// #include "rosapi_msgs/srv/detail/service_type__type_support.h"
// already included above
// #include "rosidl_typesupport_c/identifier.h"
#include "rosidl_typesupport_c/service_type_support_dispatch.h"
// already included above
// #include "rosidl_typesupport_c/type_support_map.h"
// already included above
// #include "rosidl_typesupport_interface/macros.h"
#include "service_msgs/msg/service_event_info.h"
#include "builtin_interfaces/msg/time.h"

namespace rosapi_msgs
{

namespace srv
{

namespace rosidl_typesupport_c
{
typedef struct _ServiceType_type_support_ids_t
{
  const char * typesupport_identifier[2];
} _ServiceType_type_support_ids_t;

static const _ServiceType_type_support_ids_t _ServiceType_service_typesupport_ids = {
  {
    "rosidl_typesupport_fastrtps_c",  // ::rosidl_typesupport_fastrtps_c::typesupport_identifier,
    "rosidl_typesupport_introspection_c",  // ::rosidl_typesupport_introspection_c::typesupport_identifier,
  }
};

typedef struct _ServiceType_type_support_symbol_names_t
{
  const char * symbol_name[2];
} _ServiceType_type_support_symbol_names_t;

#define STRINGIFY_(s) #s
#define STRINGIFY(s) STRINGIFY_(s)

static const _ServiceType_type_support_symbol_names_t _ServiceType_service_typesupport_symbol_names = {
  {
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_fastrtps_c, rosapi_msgs, srv, ServiceType)),
    STRINGIFY(ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_introspection_c, rosapi_msgs, srv, ServiceType)),
  }
};

typedef struct _ServiceType_type_support_data_t
{
  void * data[2];
} _ServiceType_type_support_data_t;

static _ServiceType_type_support_data_t _ServiceType_service_typesupport_data = {
  {
    0,  // will store the shared library later
    0,  // will store the shared library later
  }
};

static const type_support_map_t _ServiceType_service_typesupport_map = {
  2,
  "rosapi_msgs",
  &_ServiceType_service_typesupport_ids.typesupport_identifier[0],
  &_ServiceType_service_typesupport_symbol_names.symbol_name[0],
  &_ServiceType_service_typesupport_data.data[0],
};

static const rosidl_service_type_support_t ServiceType_service_type_support_handle = {
  rosidl_typesupport_c__typesupport_identifier,
  reinterpret_cast<const type_support_map_t *>(&_ServiceType_service_typesupport_map),
  rosidl_typesupport_c__get_service_typesupport_handle_function,
  &ServiceType_Request_message_type_support_handle,
  &ServiceType_Response_message_type_support_handle,
  &ServiceType_Event_message_type_support_handle,
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_CREATE_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    rosapi_msgs,
    srv,
    ServiceType
  ),
  ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_DESTROY_EVENT_MESSAGE_SYMBOL_NAME(
    rosidl_typesupport_c,
    rosapi_msgs,
    srv,
    ServiceType
  ),
  &rosapi_msgs__srv__ServiceType__get_type_hash,
  &rosapi_msgs__srv__ServiceType__get_type_description,
  &rosapi_msgs__srv__ServiceType__get_type_description_sources,
};

}  // namespace rosidl_typesupport_c

}  // namespace srv

}  // namespace rosapi_msgs

#ifdef __cplusplus
extern "C"
{
#endif

const rosidl_service_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__SERVICE_SYMBOL_NAME(rosidl_typesupport_c, rosapi_msgs, srv, ServiceType)() {
  return &::rosapi_msgs::srv::rosidl_typesupport_c::ServiceType_service_type_support_handle;
}

#ifdef __cplusplus
}
#endif
