  ...
  rosbridge_msgs__msg__ConnectedClient__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rosbridge_msgs__msg__ConnectedClient * data =
      (rosbridge_msgs__msg__ConnectedClient *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rosbridge_msgs__msg__ConnectedClient__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rosbridge_msgs__msg__ConnectedClient__fini(&data[i]);
        }
        free(data);
        return false;
      }
