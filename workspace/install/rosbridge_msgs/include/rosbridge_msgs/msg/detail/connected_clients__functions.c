  ...
  rosbridge_msgs__msg__ConnectedClients__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rosbridge_msgs__msg__ConnectedClients * data =
      (rosbridge_msgs__msg__ConnectedClients *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rosbridge_msgs__msg__ConnectedClients__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rosbridge_msgs__msg__ConnectedClients__fini(&data[i]);
        }
        free(data);
        return false;
      }
