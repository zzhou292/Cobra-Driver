  ...
  rosbridge_test_msgs__msg__TestFloat32BoundedArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rosbridge_test_msgs__msg__TestFloat32BoundedArray * data =
      (rosbridge_test_msgs__msg__TestFloat32BoundedArray *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rosbridge_test_msgs__msg__TestFloat32BoundedArray__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rosbridge_test_msgs__msg__TestFloat32BoundedArray__fini(&data[i]);
        }
        free(data);
        return false;
      }
