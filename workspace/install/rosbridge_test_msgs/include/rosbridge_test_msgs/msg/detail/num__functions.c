  ...
  rosbridge_test_msgs__msg__Num__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    rosbridge_test_msgs__msg__Num * data =
      (rosbridge_test_msgs__msg__Num *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rosbridge_test_msgs__msg__Num__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          rosbridge_test_msgs__msg__Num__fini(&data[i]);
        }
        free(data);
        return false;
      }
